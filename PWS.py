...
for nietwhoppa  in range(20):
    ...
    for aantalkerentesten in range(2800, 6000, 10):
        ...
        if min_min_afstandradiusmarssate > min_afstandradiusmarssate:
            ...
            print(f'De nieuwe minste afstand tot mars: {min_min_afstandradiusmarssate}')
            am = 0
        else:
            am += 1
        ...
        for WOPPA in range(5000000):
            ...
            # keplers 3rde wet
            if hoekaardemars == 180 and ak == 0:
                r1 = aarde.r_zon
                r2 = mars.r_zon
                r3 = r1 + r2
                afstanddiameter = ((r1 + r2) / 2) / au
                print(f'afstanddiameter:{afstanddiameter}')
                TijdAardeMars = math.sqrt(afstanddiameter ** 3)
                print(f'Tijdaardemars:{TijdAardeMars}')
                Tijdnaarmars = TijdAardeMars / 2
                print(f'Tijdnaarmars:{Tijdnaarmars}')
                ak = 1
            ...
            if aa > 30 and I == 0:
                ...
                # duratierondjemars
            if mars.vx > -20 and mars.vx < 20 and mars.vy > 0:
                x_mars_checkpoint1 = mars.Xpos
                y_mars_checkpoint1 = mars.Ypos
                J += 1

            if J == 1:
                Marstijdcheckpoint1 = t
            if J == 88:
                Marstijdcheckpoint2 = t
                Duratierondjemars = (Marstijdcheckpoint2 - Marstijdcheckpoint1)
                Duratierondjemarsjaar = math.sqrt(Duratierondjemars / 3600 / 24 / 365)
                print(f'Duratierondjemarsjaar:{Duratierondjemarsjaar}')

            if Duratierondjemarsjaar > 0:
                gradenvertrek1 = Tijdnaarmars / Duratierondjemarsjaar
                gradenvertrek2 = gradenvertrek1 * 360
                gradenvertrek3 = gradenvertrek2 - 180
                Gradenvertrek4 = gradenvertrek3 * -1
            ...
            if hoekaardemars > (gradenvertrekvoordesatelliet - 1.9) and hoekaardemars < (
                    gradenvertrekvoordesatelliet + 1.9) and i == 0 and S == 1 and ak == 1:
                if MarsVoor == 1 and satellietbuiten > 0 and hoekaardesatelliet < 180 and hoekaardesatelliet > 150:
                    extrav = ((math.sqrt((G * M_zon) / (r1))) * (math.sqrt((2 * r2) / (r3)) - 1)) - 50
